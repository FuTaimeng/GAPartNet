...
if args.mode == "run_arti_open_drawer_gen":
    '''
    function: init gym and run open demo
    '''
    ...
    # we choose one example object to show the demo, change the path
    paths = glob.glob(f"assets/{ROOT}/*/mobility_annotation_gapartnet.urdf")
    paths = ["/home/haoran/Projects/Part/isaacgym/assets/urdf/drawer_2.urdf"]
    for path in tqdm.tqdm(paths, total=len(paths)):
        # # get gapart id and anno
        # gapart_id = path.split("/")[-2]
        # gapart_anno_path = "/".join(path.split("/")[:-1]) + "/link_annotation_gapartnet.json"
        ...
        cfgs["asset"]["table_scale"] = [1.0, 2, 0.7]
        # cfgs["asset"]["arti_gapartnet_ids"] = [
        #     gapart_id
        # ]
        cfgs["asset"]["asset_root"] = "/home/haoran/Projects/Part/isaacgym/assets"
        cfgs["todo_urdf"] = "urdf/drawer_2.urdf"
        cfgs["asset"]["arti_obj_pose_ps"] = [[0.95, 0, -0.4*gapartnet_obj_min_z_]]
        # cfgs["cam"]["cam_poss"]=
        # init gym
        gym, cfgs = init_gym(cfgs, task_cfg=task_cfg)
        ...
        #                                         bbox_now_i[idx_set[i][1]]), dtype=np.float32), 
        #                         np.array([1, 0 ,0], dtype=np.float32))
        mesh = o3d.io.read_triangle_mesh("/home/haoran/Projects/Part/isaacgym/assets/drawer_handle/line_fixed_handle/19179_3_rot_col.obj")  # Replace with your mesh file path
        ...
        # import pdb; pdb.set_trace()
        # rotations = torch.tensor([[0, -1, 0, 0]], device='cuda:0')
        init_position = all_bbox_center_front_face[bbox_id].cpu().numpy()
        handle_out_ = handle_out[bbox_id].cpu().numpy()
...
if args.mode == "run_arti_open_door_gen":
    ...
    for path in tqdm.tqdm(paths, total=len(paths)):
        ...
        # ]
        cfgs["asset"]["asset_root"] = "/home/haoran/Projects/Part/isaacgym/assets"
        cfgs["todo_urdf"] = "urdf/door_2.urdf"
...
elif args.mode == "run_arti_open_random_drawer":
    '''
    function: init gym and run open demo
    '''
    ...
    for arti_task_cfg in tqdm.tqdm(arti_task_cfgs, total=len(arti_task_cfgs)):
        ...
        if args.save_video:
            import datetime
            current_time_str = datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
            save_video_root = f"/home/haoran_legacy/output_drawer/{gapart_id}-{drawer_link_name}-{handle_link_name}/{current_time_str}"
            os.makedirs(save_video_root, exist_ok=True)
        else:
            save_video_root = None
        cfgs["USE_CUROBO"] = False
        cfgs["asset"]["arti_obj_root"] = ROOT
        cfgs["asset"]["arti_position_noise"] = 0.05
    ...
    # for path in tqdm.tqdm(paths, total=len(paths)):
        ...
        if gapart_id in ["102278","103989","103560", "103863","103425", 
                         ...
                         "41003","46456","45203"]:
            continue
        save_dir = f"gapartnet_obj_rm_new_front/{gapart_id}"
        save_name = gapart_id
        ...
        if os.path.exists(fname):
            print("skip", fname)
            continue
        if os.path.exists(f"{save_dir}/{rm_name}/{save_name}-{rm_name}-bbox_info.npy"):
            print("skip", fname)
            continue
        cfgs = read_yaml_config(f"{args.config}.yaml")
        task_root = args.task_root
        ...
        cfgs["asset"]["arti_position_noise"] = 0.0
        cfgs["asset"]["arti_rotation_noise"] = 0
        cfgs["asset"]["arti_dof_noise"] = 0.0
...
elif args.mode == "run_arti_render_door":
    '''
    function: init gym and run render code, render the articulated object point cloud
    '''
    ROOT = "gapartnet"
    ...
    # we choose one example object to show the demo, change the path
    # paths = glob.glob(f"assets/{ROOT}/45661/remo*.urdf")
    # arti_task_cfgs = json.load(open("assets/tasks_open_door_with_handle.json", "r"))
    arti_task_cfgs = json.load(open("assets/tasks_open_slider_lid_with_handle.json", "r"))
    random.shuffle(arti_task_cfgs)
    for arti_task_cfg in tqdm.tqdm(arti_task_cfgs, total=len(arti_task_cfgs)):
        gapart_id = arti_task_cfg["id"]
        # if gapart_id in ["30666", "47711", "47712", "47713", "47714", "47715", "47716", "47717", "47718", "47719", "47720"]:
    ...
    # for path in tqdm.tqdm(paths, total=len(paths)):
        ...
        #                  "41003","46456","45203"]:
        #     continue
        save_dir = f"gapartnet_obj_rm_new_front_slilid/{gapart_id}"
...
elif args.mode == "run_arti_render_urdf":
    '''
    function: init gym and run render code, render the articulated object point cloud
    '''
    ROOT = "gapartnet"
    # read all paths
    # we choose one example object to show the demo, change the path
    # /home/haoran/Projects/Part/GAPartNet/manipulation/tmp/gapartnet/19898/remove_all.urdf
    paths = glob.glob(f"/home/haoran/Projects/Part/GAPartNet/manipulation/assets/gapartnet_rm/198*/remove_all.urdf")
    # arti_task_cfgs = json.load(open("assets/tasks_open_drawer_with_handle.json", "r"))
    # random.shuffle(arti_task_cfgs)
    # for arti_task_cfg in tqdm.tqdm(arti_task_cfgs, total=len(arti_task_cfgs)):
    for path in paths:
    ...
    # for path in tqdm.tqdm(paths, total=len(paths)):
        ...
        task_cfgs_path = "task_config.json"
        cfgs["HEADLESS"] = args.headless
        # cfgs["asset"]["asset_root"] = "/home/haoran/Projects/Part/GAPartNet/manipulation/tmp"
        cfgs["asset"]["arti_obj_root"] = "gapartnet_rm"
        cfgs["asset"]["arti_obj_pose_ps"] = [[0,0, 3]]
        cfgs["asset"]["arti_position_noise"] = 0.0
        cfgs["asset"]["arti_rotation_noise"] = 10.0
        cfgs["asset"]["arti_dof_noise"] = 0.0
